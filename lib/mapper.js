...
export function mapSlotsToRegimes(slots, citations) {
  ...
  // ── EHRC ──────────────────────────────────────────────────────────────────
  // Triggers on decision_making, customer_facing, OR description keywords that
  // indicate scoring, ranking, recommendation, screening, or filtering systems.
  // These carry equivalent equality risk to explicit decision-making systems.
  const EHRC_DESCRIPTION_KEYWORDS = [
    "score", "scoring", "rank", "ranking",
    "recommend", "recommendation",
    "personalise", "personalisation", "personalize", "personalization",
    "screen", "screening", "filter", "filtering"
  ];
  const ehrcDescriptionMatch = EHRC_DESCRIPTION_KEYWORDS.some(kw => description.includes(kw));

  if (slots.decision_making === "yes" || slots.customer_facing === "yes" || ehrcDescriptionMatch) {
    const ehrcReason = ehrcDescriptionMatch && slots.decision_making !== "yes" && slots.customer_facing !== "yes"
      ? "Your AI system performs scoring, ranking, recommendation, screening, or filtering. The Equality Act 2010 applies to any system that affects individuals differently based on protected characteristics, including systems that do not make explicit decisions. If the system produces outputs that disproportionately disadvantage users sharing a protected characteristic, that constitutes indirect discrimination regardless of intent. Ongoing bias monitoring and documentation are required."
      : "Your AI system is customer-facing or influences how individuals are treated. The Equality Act 2010 applies to any AI system that produces outputs affecting individuals or businesses, even where it does not make decisions directly. If the system produces outputs that disproportionately disadvantage users sharing a protected characteristic, that constitutes indirect discrimination regardless of intent. Ongoing bias monitoring and documentation are required.";

    triggered.push({
      ...regimes.ehrc,
      trigger_reason: ehrcReason,
    });
  }
